...
from mcp_agent.tools.claude_tool import (
    ClaudeCommandResult,
    ClaudeStreamEvent,
...
@pytest.mark.anyio
async def test_command_non_stream_success():
    tool = ClaudeTool(binary=sys.executable)
    result = await tool.run_raw("-c", "print('ok')")

    assert isinstance(result, ClaudeCommandResult)
...
async def test_streaming_yields_events_and_completion():
    ...
    assert isinstance(streamed, ClaudeStreamingResult)

    seen: list[str] = []
    async for event in streamed.stream:
        assert isinstance(event, ClaudeStreamEvent)
...
async def test_timeout_kills_process():
    tool = ClaudeTool(binary=sys.executable, default_timeout=0.01)
...
async def test_missing_binary_returns_error():
    tool = ClaudeTool(binary="claude-cli-not-installed")
